...
signal numpad_enter_pressed(num: int)

var main_theme: Theme = preload("res://main_theme.tres")
var inverse_theme: Theme = preload("res://inverse_theme.tres")
...
func delete_button_pressed() -> void:
	...
	$VBoxContainer/Number.text = $VBoxContainer/Number.text.substr(0, len($VBoxContainer/Number.text) - 1)  
	if (len($VBoxContainer/Number.text) == 0):
		number = 0
		$VBoxContainer/Number.text = "0"
	else:
		number = int($VBoxContainer/Number.text)
	
	
func enter_button_pressed() -> void:
	numpad_enter_pressed.emit(number)
	number = 0
	$VBoxContainer/Number.text = "0"
